...
export function ViewLogs() {
  ...
  const renderChangeDetails = (log: TaskLog) => {
    ...
    const hasAssigneeChange = log.details.prevAssignee !== log.details.newAssignee;
    const hasDescriptionChange = log.details.prevDescription !== log.details.newDescription;
      ...
      <div className="space-y-3 text-sm">
        ...
        {hasStatusChange && (
          <div className="flex items-center gap-2 text-sm">
            ...
            <span className="font-medium text-blue-600">{log.details.newStatus}</span>
          </div>
        )}
        ...
        {hasAssigneeChange && (
          <div className="flex items-center gap-2 text-sm">
            ...
            <ArrowRight className="h-4 w-4 text-gray-400" />
            <span className="font-medium text-blue-600">{log.details.newAssignee}</span>
          </div>
        )}
          ...
          <div className="space-y-2">
            <div className="bg-gray-50 p-3 rounded-md">
              <div className="text-gray-500">
                <span className="font-medium text-gray-700">Previous description:</span>
                <p className="mt-1">{log.details.prevDescription || 'No description'}</p>
              </div>
              <div className="mt-3 text-gray-900">
                <span className="font-medium text-gray-700">New description:</span>
                <p className="mt-1">{log.details.newDescription || 'No description'}</p>
              </div>
            </div>
          </div>
        )}
              ...
              {groupedLogs[date].map((log, index) => (
                <div key={`${log.created_at}-${index}`} className="relative pl-6 pb-8">
                  ...
                  <div className={`absolute left-[-5px] top-2 h-2.5 w-2.5 rounded-full ${
                    log.log_type === 'TASK_CREATED' ? 'bg-green-500' : 'bg-blue-500'
                  }`} />
